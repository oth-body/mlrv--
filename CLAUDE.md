...
Sample loading is in progress: `play_loop~.pd` gained a `set arrayname` method (re-points a live voice at a different loaded array without recreating it — verified against `tabread4~-help.pd`'s documented idiom, then against the real abstraction with real loaded content), and `sample_voice~.pd` exists (a gated, envelope-faded wrapper around one `play_loop~`, verified with real audio: play produces correlation-1.000000 output matching a known ramp past the fade-in, stop correctly fades to silence). `file_poly.pd` itself — the polyphonic slot bank / voice-allocation layer described in a full implementation plan in the handoff log — has not been built yet. Mapping/mode logic, mixer, pattern sequencing, and effects are not started.
